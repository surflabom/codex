...
impl ChatWidget {
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        if let Some((approval, sandbox)) = custom_config_permissions {
            ...
            let disabled_reason = match self.config.approval_policy.can_set(&approval) {
                Ok(()) => None,
                Err(err) => Some(err.to_string()),
            };
            items.push(SelectionItem {
                name: "Custom".to_string(),
                description: crate::status::custom_permissions_details(approval, &sandbox),
                is_current,
    ...
    fn custom_permissions_from_user_config(
        ...
        crate::status::custom_permissions_details(approval, &sandbox)?;
